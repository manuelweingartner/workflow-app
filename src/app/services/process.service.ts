...
export class ProcessService {
  private allDossiers = signal<Dossier[]>(ALL_DOSSIERS);
  private activeDossierId = signal<string>('1');
  ...
  readonly dossiers = this.allDossiers.asReadonly();
  readonly activeDossierIdReadonly = this.activeDossierId.asReadonly();

  readonly dossier$ = computed(() => {
    const id = this.activeDossierId();
    return this.allDossiers().find((d) => d.id === id) ?? this.allDossiers()[0];
  });

  readonly steps = computed(() => this.dossier$().process.steps);
  ...
  insertStepAfter(afterId: string) {
    const ds = structuredClone(this.allDossiers());
    const d = ds.find((x) => x.id === this.activeDossierId())!;
    const idx = d.process.steps.findIndex((s) => s.id === afterId);
    if (idx === -1) return;
    const newStep: ProcessStep = {
      ...
      status: 'pending',
      responsible: '',
      category: d.process.steps[0]?.category || 'Allgemein',
      tasks: [],
      inputs: [],
      actions: [],
      completionCriteria: [],
    ...
    d.process.steps.splice(idx + 1, 0, newStep);
    this.allDossiers.set(ds);
  ...
  updateStep(updated: ProcessStep) {
    const ds = structuredClone(this.allDossiers());
    const d = ds.find((x) => x.id === this.activeDossierId())!;
    const idx = d.process.steps.findIndex((s) => s.id === updated.id);
    if (idx !== -1) {
      d.process.steps[idx] = updated;
      this.allDossiers.set(ds);
    }
  }
  ...
  addCriterionToStep(stepId: string, description: string, suggestedNextStep?: string) {
    ...
    updated.completionCriteria.push(criterion);
    this.updateStep(updated);
  }
  ...
  completeStep(stepId: string) {
    ...
    const ds = structuredClone(this.allDossiers());
    const d = ds.find((x) => x.id === this.activeDossierId())!;
    const step = d.process.steps[idx];
    step.status = 'completed';
    step.completedDate = new Date().toLocaleDateString('de-CH');
    if (idx + 1 < d.process.steps.length) {
      const next = d.process.steps[idx + 1];
...
const DOSSIER_BAUGESUCH: Dossier = {
  id: '1',
  number: '2024-0009',
  title: 'Umbau Gebäude (Heizungsänderung und Dachstockausbau)',
  serviceRequest: {
    id: 'sr-1',
    portalFormTitle: 'Baugesuch einreichen',
    submittedDate: '20.02.2024',
    submittedBy: 'Müller Sarah',
    email: 's.mueller@example.ch',
    status: 'in-bearbeitung',
    portalStatus: 'Ihr Baugesuch wird aktuell geprüft',
    messages: [
...
const DOSSIER_AKTENEINSICHT: Dossier = {
  id: '2',
  number: '2025-0042',
  title: 'Akteneinsicht Verkehrsplanung Dorfzentrum',
  serviceRequest: {
    id: 'sr-2',
    portalFormTitle: 'Akteneinsicht beantragen',
    submittedDate: '10.03.2025',
    submittedBy: 'Keller Thomas',
    email: 't.keller@example.ch',
    status: 'in-bearbeitung',
    portalStatus: 'Ihr Antrag wird geprüft — Identitätsnachweis ausstehend',
    messages: [
...
const DOSSIER_EINBUERGERUNG: Dossier = {
  id: '3',
  number: '2025-0018',
  title: 'Einbürgerungsgesuch Rossi Marco',
  serviceRequest: {
    id: 'sr-3',
    portalFormTitle: 'Einbürgerungsgesuch stellen',
    submittedDate: '15.01.2025',
    submittedBy: 'Rossi Marco',
    email: 'm.rossi@example.ch',
    status: 'in-bearbeitung',
    portalStatus: 'Sprachprüfung ausstehend',
    messages: [
...
const DOSSIER_GEMEINDERAT: Dossier = {
  id: '4',
  number: '2025-0055',
  title: 'Anfrage Tempo-30-Zone Schulweg Birkenstrasse',
  serviceRequest: {
    id: 'sr-4',
    portalFormTitle: 'Anfrage an den Gemeinderat',
    submittedDate: '01.03.2025',
    submittedBy: 'Brunner Lisa',
    email: 'l.brunner@example.ch',
    status: 'in-bearbeitung',
    portalStatus: 'Ihre Anfrage wird dem zuständigen Ressort zugewiesen',
    messages: [
...
const DOSSIER_VERANSTALTUNG: Dossier = {
  ...
  serviceRequest: {
    id: 'sr-5',
    portalFormTitle: 'Veranstaltungsbewilligung beantragen',
    submittedDate: '20.02.2025',
    submittedBy: 'Steiner Anna',
    email: 'a.steiner@turnverein.ch',
    status: 'in-bearbeitung',
    portalStatus: 'Fachstellen werden konsultiert',
    messages: [
  ...
  process: {
    steps: [
      ...
      },
      {
        ...
        completionCriteria: [
          { id: 'va-c4', description: 'Alle Fachstellen haben Stellung genommen', met: false },
        ],
        conditionals: [],
      },
      {
      ...
      },
      {
        ...
        tasks: [
          { id: 'va-t16', title: 'Auflagen-Checkliste vor Ort prüfen', assignee: 'Frei Barbara', status: 'open' },
        ],
        inputs: [],
        actions: [],
...
const DOSSIER_KESB: Dossier = {
  id: '6',
  number: '2025-KES-0012',
  title: 'KESB-Gefahrenmeldung Fam. Schneider',
  serviceRequest: {
    id: 'sr-6',
    portalFormTitle: 'KESB-Gefahrenmeldung einreichen',
    submittedDate: '05.03.2025',
    submittedBy: 'Widmer Ruth (Schule Dorfname)',
    email: 'r.widmer@schule-dorf.ch',
    status: 'in-bearbeitung',
    portalStatus: 'Abklärung eingeleitet',
    messages: [
  ...
  process: {
    steps: [
      ...
      },
      {
        ...
        inputs: [
          { id: 'kes-i8', type: 'document', label: 'Gesprächsprotokolle', required: true, uploaded: false },
        ],
        actions: [],
      ...
      },
      {
        ...
        inputs: [
          { id: 'kes-i12', type: 'document', label: 'Verfügung', required: true, uploaded: false },
        ],
      ...
      },
      {
        ...
        inputs: [
          { id: 'kes-i13', type: 'document', label: 'Periodischer Bericht', required: true, uploaded: false },
        ],
        actions: [],

...
- [x] **KI+ Import robuster (04.09.2026)**: die Freitext-Zuordnung lief auf `domains.find(...)`, erster Treffer in Array-Reihenfolge. «Workflow für Einbürgerung erstellen» landete darum auf dem Rekrutierungsprozess, weil `er-stellen` das Stichwort `stellen` enthält. Jetzt gewichtet `matchDomain()` alle Domänen (Stichwortlänge, verdoppelt am Wortanfang) und vergleicht umlauttolerant
- [x] **KI+ Import demo-fest (15.09.2026)**: Einbürgerung hat jetzt starke Marker (`einbürger`, `eingebürgert`, `bürgerrecht`), die jede Punktzahl schlagen. Steht das Wort irgendwo im Text, kommt der Einbürgerungsprozess, unabhängig vom Verb und auch wenn noch andere Domänen im Satz vorkommen. Faltung neu auf den nackten Vokal, «Einbuergerung» und «Einburgerung» treffen ebenfalls. Ursache des Demo-Fehlers war aber der Deploy: `gh-pages` stand auf dem Build vom 03.09., der den Fix vom 04.09. nicht enthielt
- [x] **Ein Matcher für beide Einstiege (15.09.2026)**: `services/domain-match.ts` bedient Import-Dialog und KI-Assistent. Der Assistent hatte vorher eine eigene Liste ohne Einbürgerung und mit demselben Reihenfolge-Problem, im Chat kam darum die generische Vorlage
...
- [x] Sub-Prozesse
- [x] Schleifen mit Abbruchbedingung (visuell **und bedienbar**, siehe Schuleinschreibung)
